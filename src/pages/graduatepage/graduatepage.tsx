...
export default function GraduatePage() {
    ...
    (async () => {
      try {
        const [unis, svcs, cfg] = await Promise.all([
          ...
          publicTopic.getServices('graduate'),
          publicTopic.getTopicConfig('graduate'),
        ]);
  ...
  const cfg = {
    title: config?.title || '研究生专题',
    hotLabel: config?.hot_label || '研究生热门',
    payText: config?.pay_text || '咨询上岸学长学姐',
  ...
  const topUsers = useMemo(() => {
    return [...store.getUsers()].sort((a, b) => b.points - a.points).slice(0, 3);
  }, [store]);

  return (
    <div className="min-h-screen bg-gray-50 pb-4">
      <PageHeader title="研究生专题" />
                  ...
                  >
                    <div className="w-11 h-11 rounded-xl bg-blue-50 flex items-center justify-center">
                      {typeof sv.icon === 'string' ? (
                        ...
                        <sv.icon className="w-5 h-5 text-blue-600" />
                      )}
                    </div>
      ...
      <Dialog open={payOpen} onOpenChange={setPayOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              向{school?.name || ''}的上岸学长学姐发起一对一付费咨询
            </DialogDescription>
          </DialogHeader>
